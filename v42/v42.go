...
const (
	...
	NFL42_CARE_IO_ADVISE_THRU_MDS = 4
...
type Deviceid4 [NFS4_SESSIONID_SIZE]byte // deviceid4
...
type Fattr4ChangePolicy ChangePolicy4 // fattr4_change_policy
type Fattr4SecLabel SecLabel4 // fattr4_sec_label
...
type LabelformatSpec4 struct { // labelformat_spec4
	LfsLfs uint32
	LfsPi uint32
}


type SecLabel4 struct { // sec_label4
	SlaiLfs LabelformatSpec4
	SlaiData []byte
}


type CopyFromAuthPriv struct { // copy_from_auth_priv
...
type CopyToAuthPriv struct { // copy_to_auth_priv
...
type CopyConfirmAuthPriv struct { // copy_confirm_auth_priv
	CcapSharedSecretMic []byte
	CcapUsername string
}


type AppDataBlock4 struct { // app_data_block4
	AdbOffset uint64
	AdbBlockSize uint64
...
type Data4 struct { // data4
	DOffset uint64
	DData []byte
}


type DataInfo4 struct { // data_info4
	DiOffset uint64
	DiLength uint64
}


type WriteResponse4 struct { // write_response4
	WrCallbackID []Stateid4
	WrCount uint64
	WrCommitted int32
	WrWriteverf Verifier4
}
...
type CLONE4args struct { // CLONE4args
...
type CLONE4res struct { // CLONE4res
	ClStatus int32
}
...
type DestroyClientid4args struct { // DESTROY_CLIENTID4args
	DcaClientid uint64
}


type DestroyClientid4res struct { // DESTROY_CLIENTID4res
	DcrStatus int32
}
...
type COPY4args struct { // COPY4args
...
type CopyRequirements4 struct { // copy_requirements4
	CrConsecutive bool
	CrSynchronous bool
}


type COPY4resok struct { // COPY4resok
	CrResponse WriteResponse4
	CrRequirements CopyRequirements4
}


type CopyNotify4args struct { // COPY_NOTIFY4args
	CnaSrcStateid Stateid4
	CnaDestinationServer Netloc4
}


type CopyNotify4resok struct { // COPY_NOTIFY4resok
...
type OffloadCancel4args struct { // OFFLOAD_CANCEL4args
	OcaStateid Stateid4
}


type OffloadCancel4res struct { // OFFLOAD_CANCEL4res
	OcrStatus int32
}


type OffloadStatus4args struct { // OFFLOAD_STATUS4args
	OsaStateid Stateid4
}


type OffloadStatus4resok struct { // OFFLOAD_STATUS4resok
	OsrCount uint64
	OsrComplete []int32
}


type ALLOCATE4args struct { // ALLOCATE4args
	AaStateid Stateid4
	AaOffset uint64
	AaLength uint64
}
...
type ALLOCATE4res struct { // ALLOCATE4res
	ArStatus int32
}


type DEALLOCATE4args struct { // DEALLOCATE4args
	DaStateid Stateid4
	DaOffset uint64
	DaLength uint64
}
...
type DEALLOCATE4res struct { // DEALLOCATE4res
	DrStatus int32
}


type IOAdvise4args struct { // IO_ADVISE4args
	IaaStateid Stateid4
	IaaOffset uint64
	IaaCount uint64
	IaaHints []uint32
...
type IOAdvise4resok struct { // IO_ADVISE4resok
	IorHints []uint32
}


type DeviceError4 struct { // device_error4
	DeDeviceid Deviceid4
	DeStatus int32
	DeOpnum int32
}


type LAYOUTERROR4args struct { // LAYOUTERROR4args
	LeaOffset uint64
	LeaLength uint64
	LeaStateid Stateid4
...
type LAYOUTERROR4res struct { // LAYOUTERROR4res
	LerStatus int32
}


type IOInfo4 struct { // io_info4
	IiCount uint64
	IiBytes uint64
}


type LAYOUTSTATS4args struct { // LAYOUTSTATS4args
	LsaOffset uint64
	LsaLength uint64
	LsaStateid Stateid4
	LsaRead IOInfo4
	LsaWrite IOInfo4
	LsaDeviceid Deviceid4
	LsaLayoutupdate Layoutupdate4
}


type LAYOUTSTATS4res struct { // LAYOUTSTATS4res
	LsrStatus int32
}


type ReadPlus4args struct { // READ_PLUS4args
	RpaStateid Stateid4
	RpaOffset uint64
	RpaCount uint32
}
...
type ReadPlusRes4 struct { // read_plus_res4
	RprEof bool
	RprContents []ReadPlusContent
}


type SEEK4args struct { // SEEK4args
	SaStateid Stateid4
	SaOffset uint64
	SaWhat int32
}
...
type SeekRes4 struct { // seek_res4
	SrEof bool
	SrOffset uint64
}


type WriteSame4args struct { // WRITE_SAME4args
	WsaStateid Stateid4
	WsaStable int32
...
type CbOffload4args struct { // CB_OFFLOAD4args
	CoaFh NfsFh4
	CoaStateid Stateid4
...
type CbOffload4res struct { // CB_OFFLOAD4res
	CorStatus int32
}
...
type Netloc4 struct {
	NlType int32 `xdr:"union"`
	NlName  string  `xdr:"unioncase=1"` // Name
	NlUrl  string  `xdr:"unioncase=2"` // Url
	NlAddr  Netaddr4  `xdr:"unioncase=3"` // Netaddr
}
...
type Createtype4 struct {
	...
	Blkdevdata  Specdata4  `xdr:"unioncase=3"` // Nf4blk
	Chrdevdata  Specdata4  `xdr:"unioncase=4"` // Nf4chr
}
...
type COPY4res struct {
	CrStatus int32 `xdr:"union"`
	CrResok4  COPY4resok  `xdr:"unioncase=0"` // Ok
	CrRequirements  CopyRequirements4  `xdr:"unioncase=10094"` // Reqs
}


type CopyNotify4res struct {
	CnrStatus int32 `xdr:"union"`
	Resok4  CopyNotify4resok  `xdr:"unioncase=0"` // Ok
}


type OffloadStatus4res struct {
	OsrStatus int32 `xdr:"union"`
	OsrResok4  OffloadStatus4resok  `xdr:"unioncase=0"` // Ok
}


type IOAdvise4res struct {
	IorStatus int32 `xdr:"union"`
	Resok4  IOAdvise4resok  `xdr:"unioncase=0"` // Ok
}


type ReadPlusContent struct {
	RpcContent int32 `xdr:"union"`
	RpcData  Data4  `xdr:"unioncase=0"` // Data
	RpcHole  DataInfo4  `xdr:"unioncase=1"` // Hole
}


type ReadPlus4res struct {
	RpStatus int32 `xdr:"union"`
	RpResok4  ReadPlusRes4  `xdr:"unioncase=0"` // Ok
}


type SEEK4res struct {
	SaStatus int32 `xdr:"union"`
	Resok4  SeekRes4  `xdr:"unioncase=0"` // Ok
}


type WriteSame4res struct {
	WsrStatus int32 `xdr:"union"`
	Resok4  WriteResponse4  `xdr:"unioncase=0"` // Ok
}
...
type NfsArgop4 struct {
	...
	OpdestroyClientid  DestroyClientid4args  `xdr:"unioncase=57"` // Clientid
	OpreclaimComplete  ReclaimComplete4args  `xdr:"unioncase=58"` // Complete
	Opallocate  ALLOCATE4args  `xdr:"unioncase=59"` // Allocate
	Opcopy  COPY4args  `xdr:"unioncase=60"` // Copy
	OpoffloadNotify  CopyNotify4args  `xdr:"unioncase=61"` // Notify
	Opdeallocate  DEALLOCATE4args  `xdr:"unioncase=62"` // Deallocate
	OpioAdvise  IOAdvise4args  `xdr:"unioncase=63"` // Advise
	Oplayouterror  LAYOUTERROR4args  `xdr:"unioncase=64"` // Layouterror
	Oplayoutstats  LAYOUTSTATS4args  `xdr:"unioncase=65"` // Layoutstats
	OpoffloadCancel  OffloadCancel4args  `xdr:"unioncase=66"` // Cancel
	OpoffloadStatus  OffloadStatus4args  `xdr:"unioncase=67"` // Status
	OpreadPlus  ReadPlus4args  `xdr:"unioncase=68"` // Plus
	Opseek  SEEK4args  `xdr:"unioncase=69"` // Seek
	OpwriteSame  WriteSame4args  `xdr:"unioncase=70"` // Same
	Opclone  CLONE4args  `xdr:"unioncase=71"` // Clone
...
type NfsResop4 struct {
	...
	OpdestroyClientid  DestroyClientid4res  `xdr:"unioncase=57"` // Clientid
	OpreclaimComplete  ReclaimComplete4res  `xdr:"unioncase=58"` // Complete
	Opallocate  ALLOCATE4res  `xdr:"unioncase=59"` // Allocate
	Opcopy  COPY4res  `xdr:"unioncase=60"` // Copy
	OpcopyNotify  CopyNotify4res  `xdr:"unioncase=61"` // Notify
	Opdeallocate  DEALLOCATE4res  `xdr:"unioncase=62"` // Deallocate
	OpioAdvise  IOAdvise4res  `xdr:"unioncase=63"` // Advise
	Oplayouterror  LAYOUTERROR4res  `xdr:"unioncase=64"` // Layouterror
	Oplayoutstats  LAYOUTSTATS4res  `xdr:"unioncase=65"` // Layoutstats
	OpoffloadCancel  OffloadCancel4res  `xdr:"unioncase=66"` // Cancel
	OpoffloadStatus  OffloadStatus4res  `xdr:"unioncase=67"` // Status
	OpreadPlus  ReadPlus4res  `xdr:"unioncase=68"` // Plus
	Opseek  SEEK4res  `xdr:"unioncase=69"` // Seek
	OpwriteSame  WriteSame4res  `xdr:"unioncase=70"` // Same
	Opclone  CLONE4res  `xdr:"unioncase=71"` // Clone
...
type OffloadInfo4 struct {
	CoaBytesCopied uint64 `xdr:"union"`
	CoaResok4  WriteResponse4  `xdr:"unioncase=0"` // Ok
}
...
type NfsCbArgop4 struct {
	...
	Opcboffload  CbOffload4args  `xdr:"unioncase=15"` // Offload
	//    `xdr:"unioncase=10044"` // Illegal
}
...
type NfsCbResop4 struct {
	...
	Opcboffload  CbOffload4res  `xdr:"unioncase=15"` // Offload
...
func SetClientConfirm (clientid uint64, setclientidconfirm Verifier4) (NfsArgop4) {
...
func BackChannelCtl (bcacbprogram uint32, bcasecparms []CallbackSecParms4) (NfsArgop4) {
...
func Stateid (fsastateid Stateid4) (NfsArgop4) {
...
func SecinfoNoName (info int32) (NfsArgop4) {
	return NfsArgop4{Argop:52, OpsecinfoNoName: info}
}
...
func SetSsv (ssassv []byte, ssadigest []byte) (NfsArgop4) {
	return NfsArgop4{Argop:54, OpsetSsv:SetSsv4args{ SsaSsv:ssassv, SsaDigest:ssadigest } }
}


func SetStateid (tsstateids []Stateid4) (NfsArgop4) {
...
func DestroyClientid (dcaclientid uint64) (NfsArgop4) {
	return NfsArgop4{Argop:57, OpdestroyClientid:DestroyClientid4args{ DcaClientid:dcaclientid } }
}
...
func Allocate (aastateid Stateid4, aaoffset uint64, aalength uint64) (NfsArgop4) {
	return NfsArgop4{Argop:59, Opallocate:ALLOCATE4args{ AaStateid:aastateid, AaOffset:aaoffset, AaLength:aalength } }
}


func Copy (casrcstateid Stateid4, cadststateid Stateid4, casrcoffset uint64, cadstoffset uint64, cacount uint64, caconsecutive bool, casynchronous bool, casourceserver []Netloc4) (NfsArgop4) {
	return NfsArgop4{Argop:60, Opcopy:COPY4args{ CaSrcStateid:casrcstateid, CaDstStateid:cadststateid, CaSrcOffset:casrcoffset, CaDstOffset:cadstoffset, CaCount:cacount, CaConsecutive:caconsecutive, CaSynchronous:casynchronous, CaSourceServer:casourceserver } }
}


func OffloadNotify (cnasrcstateid Stateid4, cnadestinationserver Netloc4) (NfsArgop4) {
	return NfsArgop4{Argop:61, OpoffloadNotify:CopyNotify4args{ CnaSrcStateid:cnasrcstateid, CnaDestinationServer:cnadestinationserver } }
}


func Deallocate (dastateid Stateid4, daoffset uint64, dalength uint64) (NfsArgop4) {
	return NfsArgop4{Argop:62, Opdeallocate:DEALLOCATE4args{ DaStateid:dastateid, DaOffset:daoffset, DaLength:dalength } }
}


func Advise (iaastateid Stateid4, iaaoffset uint64, iaacount uint64, iaahints []uint32) (NfsArgop4) {
	return NfsArgop4{Argop:63, OpioAdvise:IOAdvise4args{ IaaStateid:iaastateid, IaaOffset:iaaoffset, IaaCount:iaacount, IaaHints:iaahints } }
}


func Layouterror (leaoffset uint64, lealength uint64, leastateid Stateid4, leaerrors []DeviceError4) (NfsArgop4) {
	return NfsArgop4{Argop:64, Oplayouterror:LAYOUTERROR4args{ LeaOffset:leaoffset, LeaLength:lealength, LeaStateid:leastateid, LeaErrors:leaerrors } }
}


func Layoutstats (lsaoffset uint64, lsalength uint64, lsastateid Stateid4, lsaread IOInfo4, lsawrite IOInfo4, lsadeviceid Deviceid4, lsalayoutupdate Layoutupdate4) (NfsArgop4) {
	return NfsArgop4{Argop:65, Oplayoutstats:LAYOUTSTATS4args{ LsaOffset:lsaoffset, LsaLength:lsalength, LsaStateid:lsastateid, LsaRead:lsaread, LsaWrite:lsawrite, LsaDeviceid:lsadeviceid, LsaLayoutupdate:lsalayoutupdate } }
}


func OffloadCancel (ocastateid Stateid4) (NfsArgop4) {
	return NfsArgop4{Argop:66, OpoffloadCancel:OffloadCancel4args{ OcaStateid:ocastateid } }
}


func OffloadStatus (osastateid Stateid4) (NfsArgop4) {
	return NfsArgop4{Argop:67, OpoffloadStatus:OffloadStatus4args{ OsaStateid:osastateid } }
}


func ReadPlus (rpastateid Stateid4, rpaoffset uint64, rpacount uint32) (NfsArgop4) {
	return NfsArgop4{Argop:68, OpreadPlus:ReadPlus4args{ RpaStateid:rpastateid, RpaOffset:rpaoffset, RpaCount:rpacount } }
}


func Seek (sastateid Stateid4, saoffset uint64, sawhat int32) (NfsArgop4) {
	return NfsArgop4{Argop:69, Opseek:SEEK4args{ SaStateid:sastateid, SaOffset:saoffset, SaWhat:sawhat } }
}


func WriteSame (wsastateid Stateid4, wsastable int32, wsaadb AppDataBlock4) (NfsArgop4) {
	return NfsArgop4{Argop:70, OpwriteSame:WriteSame4args{ WsaStateid:wsastateid, WsaStable:wsastable, WsaAdb:wsaadb } }
}


func Clone (clsrcstateid Stateid4, cldststateid Stateid4, clsrcoffset uint64, cldstoffset uint64, clcount uint64) (NfsArgop4) {
	return NfsArgop4{Argop:71, Opclone:CLONE4args{ ClSrcStateid:clsrcstateid, ClDstStateid:cldststateid, ClSrcOffset:clsrcoffset, ClDstOffset:cldstoffset, ClCount:clcount } }
}
